...
- `mouse_control.py` — Selenium CDP (Chrome DevTools Protocol) mouse control: angle-based movement, absolute positioning, boost on/off. Uses `Input.dispatchMouseEvent` to send synthetic events inside the browser without moving the OS cursor. `set_driver(driver)` must be called before use.
